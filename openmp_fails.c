...
void selectionSort(int arr[], int n)
{
	...
	for (i = 0; i < n-1; i++)
	{
		...
		//use tuple for custom reduction 
		...
		// #pragma omp parallel for firstprivate(min) lastprivate(min)
		...
		for (j = i+1; j < n; j++){
			...
			/*
			// #pragma omp single nowait { //only one thread initializes min
			// if(i==0 && j==1) { //first j loop iteration only
			// if(omp_get_thread_num==0 && i==0) { //establish static scheduling and have only the 1st thread set the initial val
				min.min_index = i;
				min.min_value=arr[i];
			}
			*/
